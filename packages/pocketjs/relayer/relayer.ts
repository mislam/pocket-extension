...
import { HTTPMethod, Node, PocketAAT, RelayHeaders, RelayPayload, Session } from '../types'
...
export class Relayer implements AbstractRelayer {
   ...
   static async relay({
      ...
      if (!this.isNodeInSession(session, serviceNode)) {
         ...
         throw new ServiceNodeNotInSessionError(`Provided node is not in the current session`)
      }
      ...
      const relayProof = {
         ...
         session_block_height: Number(session.header.sessionBlockHeight.toString()),
      ...
      const relay = await provider.relay(relayRequest, serviceNode.serviceUrl.toString(), options)
