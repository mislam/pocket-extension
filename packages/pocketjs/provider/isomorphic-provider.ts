...
export class IsomorphicProvider implements AbstractProvider {
   ...
   constructor({ rpcUrl = '', dispatchers = [] }: { rpcUrl: string; dispatchers?: string[] }) {
      this.rpcUrl = rpcUrl
   ...
   private async perform({
      ...
      const finalRpcUrl = rpcUrl
         ...
         ? this.dispatchers[Math.floor(Math.random() * 100) % this.dispatchers.length]
         : this.rpcUrl
      ...
      debug(`${routedRpcUrl} (attempt ${retriesPerformed + 1}) CALL DURATION: ${totalTime}`)
    ...
    * */
   async getTransactionCount(address: string | Promise<string>): Promise<number> {
    ...
    * */
   async getType(address: string | Promise<string>): Promise<'node' | 'app' | 'account'> {
   ...
   async getNode(address: string | Promise<string>): Promise<Node> {
      ...
      const { chains, jailed, public_key, service_url, status, tokens, unstaking_time } = node
   ...
   async getApp(address: string | Promise<string>, options: GetAppOptions): Promise<App> {
      ...
      const { chains, jailed, max_relays, public_key, staked_tokens, status } = app
